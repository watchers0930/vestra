...
## 실거래 시세 조회 계층 — 부동산 유형 커버리지 · 다중키 폴백
[coverage: high -- 2 sources]

MOLIT 실거래 조회 계층(`lib/molit/`)은 v5.174.0(2026-09-22)에서 아파트 전용 → **4개 주거유형**으로 확대됐다. 시세전망·권리분석·전세안전분석·매물시세·시세지도가 공통 소비한다. 상세 명세: `docs/ALGORITHM.md` §7.

**유형 커버리지**: 아파트/연립·다세대/단독·다가구/오피스텔 각각 매매·전월세 8개 엔드포인트. 사용자 표기는 `toResidentialType()`이 4유형으로 정규화(미지정→apartment 폴백). `fetchComprehensivePrices(addr, months, type)`는 아파트일 때만 표본 부족 시 연립·오피스텔로 보충 병합, 비아파트는 순수 조회. K-apt 단지정보는 아파트 전용이라 비아파트 시 생략(단독 시세전망은 지역평균 수준).

**계정별 구독 대응 다중키 폴백(`molitFetchRtms`)**: data.go.kr 인증키(MOLIT/KAPT 2계정)별 엔드포인트 활용신청이 달라, 우선순위 키를 순회하며 **미구독(HTTP 403 또는 200+오류XML reasonCode 30)일 때만 다음 키로 폴백**한다. `molitFetchStatus`가 HTTP 상태를 보존해 미구독(403→null)과 일시오류(5xx/타임아웃→null)를 구분: 미구독은 (endpoint,key)를 프로세스 수명 캐시에 기억해 콜드당 헛호출 1회로 제한, 일시오류는 기억하지 않아 회복 가능. 아파트 전월세는 전용키→KAPT→MOLIT 폴백으로 전세가율 누락 방지.

**시세지도 전국 커버리지(v5.177.0)**: 전국 17시도 250시군구. **강원(42→51)·전북(45→52) 자치도 코드 정정**으로 구코드 0건 잠복버그 해소(경북47·경남48은 유효). LAWD_CODE_MAP 코드 앞2자리로 시도 그룹핑한 `NATIONWIDE_SIDO_MAP`(아파트·비아파트 공통). 정적 `GU_CENTER` 없는 구는 `resolveGuCenter`가 카카오로 중심 확보+KV캐시(무데이터 시 기본좌표 폴백 방지). 상세 §7.3.

**능동 인사이트 신호(v5.176.0, `lib/proactive/`)**: 홈 진입 시 규칙 기반으로 자산·계약·등기감시·구독을 훑어 신호 추출(AI 미사용) → `briefing.ts`가 gpt-5-mini로 종합. 신호 소스: 미확인 등기변동·계약만료·구독만료·고위험/미분석 자산 + **전세가율 위험(≥90% 위험/80~90% 주의)·등기감시 미등록 자산**. 전 소스 userId 스코프·소스별 try/catch·MAX_SIGNALS 상한. 상세 §7.4.
