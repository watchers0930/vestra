...
/**
 * 로그인 후 전문가 자격 등록 카드 (4스텝 아코디언).
 * ① 분야 선택 → ② 개인정보 → ③ 전문가 정보 → ④ 심사.
 */
export default function ExpertRegisterCard({ email }: { email: string | null }) {
    ...
    >
      ...
      {email && (
        <p className="mt-2 text-xs text-gray-400">
          가입 계정 <span className="font-semibold text-gray-600">{maskEmail(email)}</span>
        </p>
      )}
