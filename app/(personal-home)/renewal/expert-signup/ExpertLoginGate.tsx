...
/**
 * 전문가 가입 로그인 게이트.
 * 컨테이너 폭 100%를 채우는 진청 히어로 안에 2칼럼(좌 설명 / 우 소셜 로그인)으로 구성.
...
export default function ExpertLoginGate() {
  ...
  return (
    ...
    >
      ...
      />
      <div className="relative p-8 sm:p-12" style={{ display: "flex", flexWrap: "wrap", gap: "2.5rem", alignItems: "flex-start" }}>
        {/* 좌: 설명 */}
        <div style={{ flex: "1 1 360px", minWidth: 0 }}>
          ...
          <h1 className="mt-5 text-[30px] font-extrabold leading-tight text-white sm:text-[38px]">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {FIELDS.map((f) => (
              <span key={f} className="rounded-full bg-white/10 px-3 py-1.5 text-xs font-medium text-white/90 backdrop-blur">
                {f}
              </span>
            ))}
          </div>
          ...
          {/* 혜택 리스트 */}
          <div className="mt-8 flex flex-col gap-5 border-t border-white/15 pt-7">
            {BENEFITS.map(({ icon: Icon, title, desc }) => (
              <div key={title} className="flex gap-3.5">
                <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-white/12 backdrop-blur">
        ...
        {/* 우: 소셜 로그인 + 절차 (흰 카드) */}
        <div className="rounded-2xl bg-white p-7 shadow-xl" style={{ flex: "0 0 360px", maxWidth: "100%" }}>
            ...
            <div className="flex flex-col gap-4">
              {STEPS.map((s) => (
                <div key={s.n} className="flex items-center gap-3">
                  <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold text-white" style={{ background: NAVY2 }}>
                    {s.n}
                  </div>
                  <div>
