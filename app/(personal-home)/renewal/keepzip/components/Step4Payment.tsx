...
interface Props {
  /** 데모: 결제 임의승인 → 발송 단계로. 실서비스는 토스 결제 confirm 연동 예정. */
  onPaid: () => void;
}
...
/** Step 4 — 결제(mock). 노쉐어: 9,900원 = 변호사 서비스료 전액 + 우체국 실비. */
  ...
  return (
    <div className={s.centerCol}>
      <div className={s.panel}>
        <div className={s.docTitle}>결제하고 발송하기</div>
        ...
        <p className={s.note}>변호사 서비스료 9,900원 + 우체국 등기 실비 포함. 서비스료 전액은 담당 변호사에게 지급됩니다.</p>

        <button className={s.submitBtn} style={{ marginTop: 12 }} disabled={paying} onClick={pay}>
          {paying ? "처리 중..." : "9,900원 결제하고 발송하기"}
        </button>
