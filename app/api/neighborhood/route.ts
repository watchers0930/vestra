...
async function kakaoGeocode(key: string, address: string): Promise<{ lat: number; lng: number; buildingName?: string; fullAddress?: string } | null> {
  try {
    ...
    const buildingName = ra?.building_name || undefined;
    // 카카오가 정규화한 전체 주소 (시군구 포함)
    const fullAddress = ra?.address_name || doc.address?.address_name || undefined;
    return { lat: parseFloat(doc.y), lng: parseFloat(doc.x), buildingName, fullAddress };
  } catch {
    return null;
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const [subway, bus, school, academy, kindergarten, mart, pharmacy, hospital, convenience, park, bank, kaptInfo] = await Promise.all([
      ...
      kakaoKeywordSearch(kakaoKey, "마트", coord, "MT1"),
      kakaoKeywordSearch(kakaoKey, "약국", coord, "PM9"),
      kakaoKeywordSearch(kakaoKey, "병원", coord, "HP8"),
