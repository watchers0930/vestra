/**
 * 국토교통부 API — 법정동 코드 맵 + XML 파싱 + 필터링 유틸
 *
 * @module lib/molit/molit-data
 */
...
export const LAWD_CODE_MAP: Record<string, string> = {
  ...
  "화성시": "41597", "화성시동탄": "41597", "양주시": "41630", "포천시": "41650",
  ...
  // ── 강원특별자치도 (51) — 2023 자치도 전환으로 42→51 (뒤 3자리 동일) ──
  "춘천시": "51110", "원주시": "51130", "강릉시": "51150", "동해시": "51170",
  "태백시": "51190", "속초시": "51210", "삼척시": "51230",
  "홍천군": "51310", "횡성군": "51330", "영월군": "51350", "평창군": "51370",
  "정선군": "51390", "철원군": "51410", "화천군": "51430", "양구군": "51450",
  "인제군": "51470", "고성군": "51480", "양양군": "51490",
  ...
  // ── 전북특별자치도 (52) — 2024 자치도 전환으로 45→52 (뒤 3자리 동일) ──
  "전주시완산구": "52111", "전주시덕진구": "52113",
...
/** 영문/한글 태그 모두 시도 */
export function extractVal(xml: string, eng: string, kor: string): string {
...
import type { RealTransaction, RentTransaction } from "../molit-api";

/** XML 응답에서 매매 거래 목록 파싱 (영문/한글 태그 호환) */
export function parseTransactions(xml: string): RealTransaction[] {
...
export function parseRentTransactions(xml: string): RentTransaction[] {
...
/**
 * 거래 목록을 동/아파트명으로 필터링
 * 단계적 필터링: 동+아파트 → 아파트만 → 동만 → 전체 반환(fallback)
 */
export function filterTransactions<T extends { dong: string; aptName: string }>(
  transactions: T[],
  dong: string | null,
  aptHint: string | null,
