...
const ENV_VARS: EnvVar[] = [
  ...
  { key: "PII_ENCRYPTION_KEY", required: true, description: "PII 암호화 마스터키(v2, AUTH_SECRET과 독립). 미설정 시 PII 저장/조회 전 경로가 런타임 throw" },
  { key: "SEARCH_INDEX_KEY", required: false, description: "blind index 전용 HMAC 키(미설정 시 AUTH_SECRET 폴백). 운영은 전용키 백필 완료 상태여야 함" },
