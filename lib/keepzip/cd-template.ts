...
- 기한 내 반환이 이루어지지 않을 경우, 반환 지체일부터 실제 반환일까지 미반환 보증금에 대하여 민법 제379조에서 정한 연 5%의 법정이율에 의한 지연손해금(금융이자)을 함께 청구한다는 취지를, 본문에 반드시 별도의 번호 항목으로 명시한다(이율 "연 5%"를 문장에 그대로 기재).
