...
- 기한 내 반환이 이루어지지 않을 경우 미반환 보증금에 대하여 반환 지체일부터 실제 반환일까지 민법 제379조가 정한 연 5%의 법정이율에 의한 지연손해금을 함께 청구한다는 취지를, 본문에 별도의 번호 항목으로 넣되 반드시 한 문장(한 줄)으로 간결히 작성한다(가·나 등 하위 항목으로 나누지 말 것, 이율 "연 5%"는 문장에 그대로 기재).
