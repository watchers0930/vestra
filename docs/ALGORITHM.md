...
### 7.3 시세지도 전국 커버리지 (v5.177.0)

시세지도(`price-map`)를 전국 **17시도 250시군구**로 확대. LAWD_CODE_MAP·주소검색·geocoding은 이미 전국을 수용했고 UI 드롭다운이 병목이었다.

- **법정동코드 정정(자치도 전환)**: 강원도→강원특별자치도(2023)로 코드 앞2자리 **42→51**, 전북도→전북특별자치도(2024)로 **45→52** (뒤 3자리 동일). 구코드로 조회하면 해당 지역 실거래가 0건이 되므로 정정. (경북 47·경남 48은 자치도 전환 없어 유효)
- **전국 드롭다운**: LAWD_CODE_MAP을 코드 앞2자리(=시도)로 그룹핑해 `NATIONWIDE_SIDO_MAP`(250구) 생성. 아파트·비아파트 공통. 중구·남구 등 도시 간 이름 충돌은 도시 접두사 키로 구분.
- **동적 구 중심 좌표**: 정적 `GU_CENTER` 미보유 구는 `resolveGuCenter`가 카카오 주소검색으로 중심을 구해 KV 캐시(반경 바이어스 + 무데이터 시 기본 좌표 폴백 방지).

### 7.4 능동 인사이트 신호 소스 (v5.176.0)

홈 진입 시 사용자 상태를 규칙 기반으로 훑어 "지금 챙길 것"을 신호로 추출(`lib/proactive/signals.ts`, AI 미사용). AI 종합은 `briefing.ts`가 담당. DB만으로 감지하는 저비용 신호(홈 실시간 조회 안전):

- 미확인 등기변동 · 계약 만료 임박 · 구독 만료 임박 · 고위험/미분석 자산
- **전세가율 위험**(전세보증금/시세 ≥90% 위험/80~90% 주의) · **등기감시 미등록 자산**(분석 완료했으나 감시 미설정 → 등기감시 권유, 정규화 주소 매칭·중복 제거)
- 모든 소스는 userId 스코프·소스별 try/catch·MAX_SIGNALS 상한.
