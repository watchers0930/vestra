...
> **전제**: 추천 커미션은 **부동산 회원(월 15만원)에만** 발생(개인·전문가 구독은 대상 아님). 유치 1인당 인센티브 풀 5만원. **품질필터·첫 달 혜택 반영 전 이론 최대치**이며, 등급 분포는 **예시 가정**이다(분포에 따라 커미션은 매출의 대략 20~27% 범위에서 변동).

### 12.1 부동산 100명 규모 — 두 분포 비교

**시나리오 A — 분산형 (여러 추천인이 골고루)**

| 등급 | 추천인 | 1인 유치 | 유치 합 | 요율 | 커미션 | 본사 몫 |
|---|---|---|---|---|---|---|
| 엘리트 | 1 | 50 | 50 | 5만 | 250만 | 500만 |
| 프로 | 2 | 12 | 24 | 3만 | 72만 | 288만 |
| 플러스 | 2 | 6 | 12 | 2만 | 24만 | 156만 |
| 베이직 | 4 | 1 | 4 | 1만 | 4만 | 56만 |
| 직접가입 | — | — | 10 | — | 0 | 150만 |
| **합계** | 9명 | | **100명** | | **350만** | **1,150만** |

→ 월 매출 1,500만 · 커미션 350만(**23.3%**) · 본사 수취 **1,150만**

**시나리오 B — 상위 집중형 (소수 슈퍼 추천인)**

| 등급 | 추천인 | 1인 유치 | 유치 합 | 요율 | 커미션 | 본사 몫 |
|---|---|---|---|---|---|---|
| 엘리트 | 1 | 50 | 50 | 5만 | 250만 | 500만 |
| 프리미엄 | 1 | 30 | 30 | 4만 | 120만 | 330만 |
| 프로 | 1 | 10 | 10 | 3만 | 30만 | 120만 |
| 직접가입 | — | — | 10 | — | 0 | 150만 |
| **합계** | 3명 | | **100명** | | **400만** | **1,100만** |

→ 월 매출 1,500만 · 커미션 400만(**26.7%**) · 본사 수취 **1,100만**

> 💡 100명 규모에선 엘리트 1명(유치 50명)이 시장 절반을 점유한다. 5개 등급을 평면으로 최소 1명씩 다 채우려면 최소 102명(엘리트50+프리미엄30+프로10+플러스5+베이직2+추천인 5)이 필요해, 100명 규모에선 상위 2개 등급 동시 배치가 구조적으로 빠듯하다. 회원이 200~300명 이상이어야 등급 분포가 자연스러워진다.

### 12.2 부동산 단일 상품 — 규모 확장 (커미션 ≈27% 가정)

| 회원 수 | 월 매출 | 커미션 | 본사 수취(월) | 본사 수취(연) |
|---|---|---|---|---|
| 100명 | 1,500만 | 400만 | 1,100만 | 1.32억 |
| 300명 | 4,500만 | 1,200만 | 3,300만 | 3.96억 |
| 500명 | 7,500만 | 2,000만 | 5,500만 | 6.6억 |
| 1,000명 | 1.5억 | 4,000만 | 1.1억 | 13.2억 |

### 12.3 종합 시뮬레이션 (부동산 500 + 개인 1,000 + 전문가 50)

**부동산 500명 등급 분포 (예시)**

| 등급 | 추천인 | 1인 유치 | 유치 합 | 요율 | 커미션 |
|---|---|---|---|---|---|
| 엘리트 | 3 | 50 | 150 | 5만 | 750만 |
| 프리미엄 | 4 | 35 | 140 | 4만 | 560만 |
| 프로 | 6 | 15 | 90 | 3만 | 270만 |
| 플러스 | 8 | 7 | 56 | 2만 | 112만 |
| 베이직 | 20 | 2 | 40 | 1만 | 40만 |
| 직접가입 | — | — | 24 | — | 0 |
...
| 구분 | 인원 | 단가 | 월 매출 | 추천 커미션 | 본사 수취 |
|---|---|---|---|---|---|
| 부동산 | 500명 | 15만원 | 7,500만 | 1,732만 | 5,768만 |
| 개인 | 1,000명 | 9,900원 | 990만 | 0 | 990만 |
| 전문가 | 50명 | 5만원 | 250만 | 0 | 250만 |
| **합계** | **1,550명** | | **8,740만** | **1,732만** | **7,008만** |

→ **월 총매출 8,740만 · 월 본사 수취 7,008만** / **연 환산: 매출 약 10.5억 · 본사 수취 약 8.4억 · 커미션 약 2.1억**

### 12.4 ⚠️ 해석 시 주의

1. **"본사 수취" ≠ 순이익.** 여기서 서버·OpenAI API·틸코·결제수수료·인건비·세금 등 **운영 원가를 추가로 차감**해야 실제 순이익이다.
2. **커미션 금액은 가정한 등급 분포에 의존**한다(상위 등급 비중↑ → 커미션↑, 대략 매출의 20~27%).
3. **개인·전문가 매출은 커미션이 없어 마진이 가장 좋은 순수 매출**이다.
4. 위 수치는 **이론 최대치** — 실제로는 첫 달 혜택월(커미션 미발생)·이탈·품질필터로 다소 낮아진다.
