	...
	const cache = (function() {
		...
		const createProductItemContainerHtml = (children) => {
			...
			const createProductItemHtml = (product) => {
				const safeLinkUrl = escapeHtml(safeUrl(product.link_url));
				const safeCode = escapeHtml(product.code);
				const safeThumbnail = escapeHtml(safeUrl(product.thumbnail));
				const safeName = escapeHtml(product.name);

				return html`
					<a class="recent-product__item-root" href="${safeLinkUrl}" data-code="${safeCode}">
						<div class="recent-product__item-img-wrapper">
							<img src="${safeThumbnail}" alt="${safeName}" class="recent-product__item-img" />
						</div>
						<p class="recent-product__item-name">${safeName}</p>
					</a>
				`;
			}
