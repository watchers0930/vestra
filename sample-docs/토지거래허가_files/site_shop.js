...
var SITE_SHOP_DETAIL = function () {
		...
		const loadingImageJobs = Array.prototype.map.call(imgElements, (async (imgElement) => {
			...
			try {
				...
				if (srcResized.pathname.endsWith('.gif')){
					srcResized.searchParams.set('f', 'gif');
				}
    ...
    */
            ...
            switch (type) {
                case 'prod':
                    ...
                    if(membership_discount_not_involved_price > 0){
                        // 등급할인 적용 시 전체 가격에서 등급할인 미적용 상품 가격 제외
                    ...
                    if (period_discount_price > 0 && show_period_discount == false) {
                        // 등급할인 적용시 즉시기간할인된 금액은 먼저 제외한 금액으로 계산
                        membership_discount_target_price -= period_discount_price;

                        // @FIXME 등급할인은 미적용인데 즉시기간은 적용된 상품의 즉시기간할인금액이 같이 마이너스되면 계산이 틀어질 수도 있어 수정필요
                    }

                    membership_discount_price = calcMembershipDiscount(membership_discount_target_price);
	...
	async function selectFreebieAsync () {
		...
		if (cart_type === 'regularly' || !checkRequireOption()) {
		    ...
		    selectAdditionalProduct: (product_idx, price, name, maximum_purchase_quantity, member_maximum_purchase_quantity, maximum_purchase_quantity_type, optional_limit, optional_limit_type, stock_use, stock, stock_unlimit, option_mix_type, prod_type, period_discount_flag, period_discount_data, period_discout_group_list, membership_discount) => {
			    ...
			    // PC: 메인 옵션은 리로드하지 않음 (리로드시 추가상품 영역이 다시 그려져 인라인 옵션이 사라짐)
				    ...
				    try{
				        ...
				        var $listItem = ($clicked && $clicked.length) ? $clicked.closest('.list-item') : $();
				        if($listItem.length){
				            var $inline = $('<div class="_prod_additional_inline_options"></div>');
...
var SITE_SHOP_CART = function () {
    ...
    var add_order_progress_check = false;
    var cart_type = '';
    var cartPromotionModalDescription = '조건 충족 시 아래 상품 중 1개를<br/>첫 구매 혜택가에 구매할 수 있어요!';
    ...
    var setCartPriceHtml = function (data) {
        ...
        var periodPriceExist = (data['period_price_exist'] === true || data['period_price_exist'] === 1 || data['period_price_exist'] === '1');
        var periodPricePcExist = (data['period_price_pc_exist'] === true || data['period_price_pc_exist'] === 1 || data['period_price_pc_exist'] === '1');
        var periodPrice = data['period_price'];
        var periodPricePc = data['period_price_pc'] || periodPrice;
        $cart_list_wrap.find('._cart_period_field').toggle(periodPriceExist);
        $cart_list_wrap.find('._cart_period_discount').text(periodPrice);
        var periodPricePcVisible = periodPricePcExist || (periodPricePc && periodPricePc !== '-');
        $pc_cart_result_container.find('._cart_period_field').toggle(periodPricePcVisible);
        $pc_cart_result_container.find('._cart_period_discount').text(periodPricePc);
        $cart_list_wrap.find('._cart_deliv_price_field').toggle(data['count'] !== data['no_deliv_count']);
        if (data['membership_price_exist'] === false && periodPricePcVisible === false && (data['count'] === data['no_deliv_count'])) {
    ...
    var setCartBenefitInfo = function (data) {
        ...
        var $mobileOrderPrice = $cart_list_wrap.find('._cart_mobile_order_price');
        ...
        $cart_list_wrap.find('._cart_mobile_period_discount_field').show();
        ...
        if (!benefitInfo || benefitInfo['promotion_exists'] !== true) {
            cartPromotionModalDescription = '조건 충족 시 아래 상품 중 1개를<br/>첫 구매 혜택가에 구매할 수 있어요!';
            return;
        }
        ...
        var benefitShortfallAmount = parseInt(benefitInfo['benefit_shortfall_amount'] || 0, 10) || 0;
        var benefitShortfallQuantity = parseInt(benefitInfo['benefit_shortfall_quantity'] || 0, 10) || 0;
        if ($promotionModalShell.length > 0) {
            if (benefitShortfallAmount > 0) {
                cartPromotionModalDescription = '<strong>' + LOCALIZE.getCurrencyFormat(benefitShortfallAmount) + '</strong> 더 담으면 아래 상품 중 1개를<br/>첫 구매 혜택가에 구매할 수 있어요!';
                $promotionModalShell.attr('data-description', cartPromotionModalDescription);
            } else if (benefitShortfallQuantity > 0) {
                cartPromotionModalDescription = '<strong>' + benefitShortfallQuantity.toLocaleString('ko-KR') + '개</strong> 더 담으면 아래 상품 중 1개를<br/>첫 구매 혜택가에 구매할 수 있어요!';
                $promotionModalShell.attr('data-description', cartPromotionModalDescription);
            } else {
                cartPromotionModalDescription = '조건 충족 시 아래 상품 중 1개를<br/>첫 구매 혜택가에 구매할 수 있어요!';
                $promotionModalShell.attr('data-description', cartPromotionModalDescription);
        ...
        if ($mobileOrderPrice.length > 0) {
            $mobileOrderPrice.each(function () {
                var $price = $(this);
                var originPrice = parseInt($price.data('discounted-price'), 10);
                if (!isNaN(originPrice) && originPrice >= 0) {
                    $price.text(LOCALIZE.getCurrencyFormat(originPrice));
                }
            });
        }
        if ($pcPriceDefault.length > 0) {
            $pcPriceDefault.each(function () {
                var $price = $(this);
                var originPrice = parseInt($price.data('discounted-price'), 10);
                if (!isNaN(originPrice) && originPrice >= 0) {
                    $price.find('._cart_price_default_value').text(LOCALIZE.getCurrencyFormat(originPrice));
                }
            });
        }
        ...
        $benefitItems.each(function () {
            ...
            if ($row.length > 0) {
                if (isQualified && benefitMap[prodCode]['is_applied'] === true) {
                    ...
                    var $pcOrderPriceOrigin = $row.find('._cart_price_origin');
                    var pcDiscountedPrice = parseInt($pcOrderPrice.data('discounted-price'), 10);
                    var pcBenefitPriceText = '';
                    var hasPcBenefitPrice = false;
                    if (!isNaN(pcDiscountedPrice) && benefitDiscountPrice > 0) {
                        var pcNewPrice = pcDiscountedPrice - benefitDiscountPrice;
                        if (pcNewPrice < 0) {
                            pcNewPrice = 0;
                        }
                        pcBenefitPriceText = LOCALIZE.getCurrencyFormat(pcNewPrice);
                        hasPcBenefitPrice = true;
                        $pcOrderPrice.find('._cart_price_default_value').text(pcBenefitPriceText);
                    } else if (priceText) {
                        pcBenefitPriceText = priceText;
                        hasPcBenefitPrice = true;
                    }

                    if (hasPcBenefitPrice) {
                        var pcOriginPrice = parseInt($pcOrderPriceOrigin.data('origin-price'), 10);
                        if (!isNaN(pcOriginPrice) && pcOriginPrice >= 0) {
                            $pcOrderPriceOrigin.text(LOCALIZE.getCurrencyFormat(pcOriginPrice)).show();
                        } else {
                            $pcOrderPriceOrigin.show();
                        }
                    ...
                    if (benefitDiscountPrice > 0 && benefitDiscountText) {
                        ...
                        var $orderPrice = $row.find('._cart_mobile_order_price');
                        var discountedPrice = parseInt($orderPrice.data('discounted-price'), 10);
                        var newPrice = null;
                        if (!isNaN(discountedPrice)) {
                            newPrice = discountedPrice - benefitDiscountPrice;
                            if (newPrice < 0) {
                                newPrice = 0;
                            }
                            $orderPrice.text(LOCALIZE.getCurrencyFormat(newPrice));
                        }
                        var $mobileOriginPrice = $row.find('._cart_mobile_order_price_origin');
                        var mobileOriginPrice = parseInt($mobileOriginPrice.data('origin-price'), 10);
                        if (!isNaN(mobileOriginPrice) && mobileOriginPrice >= 0) {
                            $mobileOriginPrice.text(LOCALIZE.getCurrencyFormat(mobileOriginPrice)).show();
                        } else {
                            $mobileOriginPrice.show();
                        }
                    }
                } else if (!isQualified) {
                    if (priceText) {
                        ...
                        var pcBenefitLinkHtml = '<a href="#" onclick="SITE_SHOP_CART.openPromotionModal(); return false;" class="_cart_promotion_modal_link" style="' + benefitLinkStyle + '">첫 구매 혜택가 ' + priceText + benefitInfoIcon + '</a>';
                        var mobileBenefitLinkHtml = '<a href="#" onclick="SITE_SHOP_CART.openPromotionModal(); return false;" class="_cart_promotion_modal_link" style="' + benefitLinkStyle + '">첫 구매 혜택가 ' + priceText + benefitInfoIcon + '</a>';
    ...
    const removeCartItemOption = (item_code, optionNo, is_regularly) => {
        $.ajax({
            ...
            success: ({ msg }) => {
                ...
                const optCount = parseInt($card.find('._counter_value').text(), 10) || 0;
                ...
                $pcDefault.data('discounted-price', newDiscounted);
                $row.find('._cart_price_default_value').text(LOCALIZE.getCurrencyFormat(newDiscounted));

                const $pcOrigin = $row.find('._cart_price_origin');
                const newOrigin = (parseInt($pcOrigin.data('origin-price'), 10) || 0) - originDelta;
                $pcOrigin.data('origin-price', newOrigin).text(LOCALIZE.getCurrencyFormat(newOrigin));
                $row.find('._cart_item_price').text(LOCALIZE.getCurrencyFormat(newOrigin));

                const hasDiscount = newOrigin !== newDiscounted;
                $pcOrigin.data('default-visible', hasDiscount ? 'Y' : 'N').toggle(hasDiscount);

                // 모바일 가격 갱신
                $row.find('._cart_mobile_order_price').text(LOCALIZE.getCurrencyFormat(newDiscounted)).data('discounted-price', newDiscounted);
                const $mobileOrigin = $row.find('._cart_mobile_order_price_origin');
                $mobileOrigin.text(LOCALIZE.getCurrencyFormat(newOrigin)).data('origin-price', newOrigin);
     ...
     */
    var addOrderWithCart = async function (type, item_code, backurl, direct, is_oms, params, is_gift_buy){
			...
			const { freebiesUserSelected, selectedAllFreebiesRequired } = await new Promise((resolve) => {
				...
				const timeoutId = setTimeout(() => {
					if (settled) return;
					settled = true;
					// 500ms 내 MFE 응답 없으면 사은품 없이 주문 진행 (사은품 미사용 사이트 포함)
					resolve({ freebiesUserSelected: [], selectedAllFreebiesRequired: true });
				}, 500);
    ...
    return {
        ...
        // 장바구니 목록에서 수량 직접 변경 (옵션/단일상품 공통, debounce 적용)
        directChangeCount(el, delta) {
            ...
            const currentCount = parseInt($val.text()) || 1;
            ...
            const { mode, itemCode: item_code, isRegularly: is_regularly, isC3: is_c3, optionNo } = $counter.data();
            const option_no = (mode === 'option_count') ? optionNo : -1;
            const timerKey = `${item_code}_${option_no}`;
            ...
            this._debounce(timerKey, () => {
                ...
                const finalCount = parseInt($val.text()) || 1;

                const postData = { mode, item_code, count: finalCount, is_regularly, item_list: selectedCartItem };
                        ...
                        if (result.msg === 'SUCCESS') {
                            ...
                            if (result.item_prices) {
                                $.each(result.item_prices, (itemCode, info) => {
                                    ...
                                    $itemRow.find('._cart_price_default_value').text(info.price);
                                    $itemRow.find('._cart_price_origin').text(info.origin_price);
                                    $itemRow.find('._cart_mobile_order_price').text(info.price);
                                    $itemRow.find('._cart_mobile_order_price_origin').text(info.origin_price);
                                    $itemRow.find('._cart_price_default').data('discounted-price', info.price_raw);
                                    $itemRow.find('._cart_mobile_order_price').data('discounted-price', info.price_raw);
                                    $itemRow.find('._cart_price_origin').data('origin-price', info.origin_price_raw);
                                    $itemRow.find('._cart_mobile_order_price_origin').data('origin-price', info.origin_price_raw);
                                    const hasDiscount = info.origin_price_raw !== info.price_raw;
                                    $itemRow.find('._cart_price_origin').data('default-visible', hasDiscount ? 'Y' : 'N');
                                    $itemRow.find('._cart_mobile_order_price_origin').data('default-visible', hasDiscount ? 'Y' : 'N');

                                    // PC 배송비 업데이트
                                    if (info.deliv_price_text !== undefined && info.deliv_price_text !== '') {
                                        const $delivTd = $cart_list_wrap.find(`._cart_item_deliv_td[data-item-code="${itemCode}"]`);
                                        if ($delivTd.length) {
                                            $delivTd.find('._cart_item_deliv_price span.text-bold').html(info.deliv_price_text);
                                        }
                                        // 모바일 개별 배송비 업데이트
                                    ...
                                    if (info.bundle_deliv_text !== undefined && info.bundle_deliv_text !== '') {
                                        const $shippingRow = $cart_list_wrap.find(`tr.im-tr-shipping[data-item-code="${itemCode}"]`);
                                        if ($shippingRow.length) {
                                            $shippingRow.find('._cart_bundle_deliv_text').html(info.bundle_deliv_text);
                                        }
                                    }
                                    // 모바일 아이템별 상품 할인금액 업데이트
                                    if (info.period_discount !== undefined) {
                                        ...
                                        if (info.period_discount > 0) {
                                            $periodPrice.data('period-discount-price', info.period_discount);
                                            $periodPrice.text('- ' + LOCALIZE.getCurrencyFormat(info.period_discount));
                                            $periodField.show();
                                        } else {
