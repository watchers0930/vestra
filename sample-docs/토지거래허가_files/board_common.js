					...
					if(res.msg === 'SUCCESS'){
						...
						$('._comment_body_' + writer).html("<p class='text-gray'><i aria-hidden='true' class='icon icon-exclamation' style='margin-right: 5px;'></i>" + getLocalizeString("설명_차단된작성자의댓글입니다", "", "차단된 작성자의 댓글입니다.") + "</p>");
							...
							$('._block_'+ writer)
								...
								.on('click', function(e){
									e.preventDefault();
									BOARD_COMMON.deleteBlock(res.post_code, writer, 'comment', bord_code, current_page);
								});
						...
						$('.margin-top-xxl._comment_body_' + writer).html("<p class='text-gray'><i aria-hidden='true' class='icon icon-exclamation' style='margin-right: 5px;'></i>" + getLocalizeString("설명_차단된작성자의게시글입니다", "", "차단된 작성자의 게시글입니다.") + "</p>");
							...
							$('._board._block_'+ writer)
								...
								.on('click', function(e){
									e.preventDefault();
									BOARD_COMMON.deleteBlock(res.post_code, writer, 'post', bord_code, current_page);
								});
				...
				success : function(res){
					if(res.msg === 'SUCCESS'){
						...
						$('._board._block_'+ writer)
							...
							.on('click', function(e){
								e.preventDefault();
								BOARD_COMMON.addBlock(res.code, writer, 'post', bord_code);
							});
					}else{
						alert(res.msg)
					}
